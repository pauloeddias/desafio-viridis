...
### Load the data
ctrain <- read.csv("temptrain.csv")
ctest <- read.csv("temptest.csv") 
c_all <- rbind(ctrain,ctest)
### Fit the ARIMA model
arima <- arima(ctrain$LandAverageTemperature, 
               order=c(1, 1, 1), 
               seasonal=list(order=c(0,1,1), period=12))

### Actual versus predicted
d1 <- data.frame(c(fitted(arima), # fitted and predicted
                   predict(arima, n.ahead = 431)$pred),
                 as.numeric(c_all$LandAverageTemperature), #actual values
                 c_all$dt)

names(d1) <- c("Fitted", "Actual", "Date")

write.csv(d1,"temp_arima.csv")
