...
### Load the data
ctrain <- read.csv("support_train.csv")
ctest <- read.csv("support_test.csv") 
c_all <- rbind(ctrain,ctest)
### Fit the ARIMA model
arima <- arima(ctrain$temp,
               xreg = ctrain$carbon,
               order=c(2, 1, 2),
...
#                    predict(arima,h=420,newxreg = ctest$carbon)$pred),
#                  as.numeric(c_all$temp), #actual values
#                  c_all$datetime)
# 
# names(d1) <- c("Fitted", "Actual", "Date")

#write.csv(d1,"temp_support.csv")
